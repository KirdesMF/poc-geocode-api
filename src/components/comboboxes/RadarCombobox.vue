...
import { radarQuery } from '@/api/queries';
...
const { data, isFetching } = useQuery(
  radarQuery({ query: debouncedQuery, language, limit: DEFAULT_LIMIT }),
);

const collection = computed(() =>
  createListCollection({
    items: data?.value?.addresses ?? [],
    itemToString: (item) => item.formattedAddress,
    itemToValue: (item) => item.longitude + ',' + item.latitude,
  }),
);
...
<template>
  <Combobox.Root :collection="collection" @input-value-change="handleInputChange">
    <Combobox.Control class="w-full p-2 border border-gray-200 rounded-md flex gap-2 items-center">
